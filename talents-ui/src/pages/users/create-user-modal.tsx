...
export default class CreateUserModal extends Component<IProps, IState>{
    ...
    componentDidMount = async () => {
        ...
        const roles = await UserService.roleItems(0);
        this.setState({ roles, isLoading: false, showSpinner: false });
        const roleIds: (number | undefined)[] = roles.map((r: ICreateUpdateUserRoleItem) => {
            if (r.isChecked) return r.roleId;
        });
        this.formRef.current?.setFieldsValue({ roles: roleIds });
    }
                ...
                <Modal
                    ...
                    footer={
                        [
                            ...
                            <Button icon={<SaveFilled />} form="createUserForm" htmlType="submit" key="submit" type="primary" loading={this.state.isLoading} onClick={this.props.onOk}>
                                Save
                            </Button>,
                        ]
                    }>
                        ...
                        <fieldset disabled={this.state.isLoading}>
                            <Form
                                ...
                                initialValues={{
                                    isActive: true,
                                    setRandomPassword: true,
                                    shouldSetPasswordOnNextLogin: true,
